<?
	...
	//Fehlerquelle: AggType ist "Gr��er" als der angezeigte Zeitraum
	// vielleicht alles als cfg direkt json_encoden und nicht jedes Teil einzeln

	//--------------------------------------------------------------------------------------------------------------------------------
	// F�r die Darstellung der Graphen wird das HTML5/JS Framework "Highcharts" der Fa. Highslide Software verwendet (www.highcharts.com)
	...
	// Download: wwww.highcharts.com/download/ ... und die Dateien einfach in das Webfront (Es sollte ein V 2.2 oder h�her verwendet werden.
   ...
   function GetHtmlForTheme($cfg){
 	...
 	function CheckCfgDaten($cfg)	{
      ...
      DebugModuleName($cfg, "CheckCfgDaten");
   	...
   	if (!in_array($cfg['Ips']['ChartType'], array("Highcharts", "Highstock")))
		   die ("Abbruch! F�r ChartType sind nur folgende M�glichkeiten zul�ssig: Highcharts, Highstock");

		// RunMode
		IfNotIssetSetValue($cfg['RunMode'], 								"script");

   	if (!in_array($cfg['RunMode'], array("script", "file", "popup")))
		   die ("Abbruch! F�r RunMode sind nur folgende M�glichkeiten zul�ssig: Script, File, Popup");
	...
	function CompatibilityCheck($cfg)	{
		...
		if (isset($cfg['Series']) && isset($cfg['series']))
			die ("Abbruch - Es d�fen nicht gleichzeitig 'Series' und 'series' definiert werden.");
		if (isset($cfg['Series']) && !isset($cfg['series']))
	...
	// ------------------------------------------------------------------------
	// CheckCfg
	//    Pr�ft die Konfiguration und korrigiert und Vervollst�ndigtdiese zum Teil
	//    IN: $cfg = ..
	...
	// ------------------------------------------------------------------------
	// CheckCfg_Common
	//    wenn RunMode=Popup, pr�fen der dazugeh�rigen Daten wie WebfrontConfigId, usw.
	//		und wenn RunMode=Popup, pr�fen der dazugeh�rigen Daten wie WebfrontConfigId, usw.
	//    IN: $cfg
	...
	function CheckCfg_Common($cfg)
 	{
      DebugModuleName($cfg,"CheckCfg_Common");
		...
		if ($cfg['RunMode'] == "popup")
		{
			...
			// pr�fen ob die �bergebene Id ein WebFront ist
	...
	//    pr�fen ob Angaben der Content Variable stimmen oder ob es das �bergeordnete Element ist
	//    IN: $cfg
	//    OUT: korrigiertes $cfg
	// ------------------------------------------------------------------------
	function Check_ContentVariable($cfg, $scriptId)
	{
	   DebugModuleName($cfg,"Check_ContentVariable");
		...
		// wenn keine Id �bergeben wurde wird das �bergeordnete Objekt als Content verwendet
	...
	//    Start- und Endzeit des gesamten Charts errechnen, und an jede Serie �bergeben
	//    IN: $cfg
	//    OUT: korrigiertes $cfg
	// ------------------------------------------------------------------------
	function CheckCfg_StartEndTime($cfg)
	{
		...
		for ($i = 0; $i < $Count; $i++)
		{
			...
			// wenn f�r die Serie keine Start oder Endzeit �bergeben w�rde wird der Standardwert genommen
	...
	function CheckCfg_Series($cfg)
 	{
		...
		foreach ($cfg['series'] as $Serie)
		{
			...
			// hier wird nur gepr�ft ob Wert von Eingabe passen k�nnte (wenn vorhanden)
			...
			// �ber AggValue kann Min/Max oder Avg vorgew�hlt werden (zum Lesen der AggregValues)
			IfNotIssetSetValue($Serie['AggValue'], "Avg");

			if ($Serie['AggValue'] != "Avg"
				&& $Serie['AggValue'] != "Min"
				&& $Serie['AggValue'] != "Max")
			   	die ("Abbruch - 'AggValue' hat keinen g�ltigen Wert");

			// Offset f�r Darstellung von z.B. Monate und Vormonat in einem Chart
			IfNotIssetSetValue($Serie['Offset'], 0);

			IfNotIssetSetValue($Serie['ReplaceValues'], false);

			// Name (Kompatibilit�t aus V1.x)
			...
			//KHC 28.04.2014 keine Pr�fung des Types
			// type & Parameter
			if (isset($Serie['type']) && isset($Serie['Param']))
				die ("Abbruch - Definition von 'Param' und 'type' in Serie gleichzeitig nicht m�glich.");
			if (!isset($Serie['type']) && !isset($Serie['Param']))
			...
			if (isset($Serie['Data']) && isset($Serie['data']))
				die ("Abbruch - Definition von 'Data' und 'data' in ein und derselben Serie nicht m�glich.");
			if (!isset($Serie['data']) && isset($Serie['Data']))
         {
			...
			// diverse Pr�fungen bei PIE-Charts
			if ($Serie['Ips']['Type'] == 'pie')
			{
            if (isset($Serie['Id']))
            {
					if (!isset($Serie['AggType']))
						die ("Abbruch - Wird ein Pie �ber Id definiert muss auch AggType parametriert werden");

					// wenn nichts angegeben wird 'AggNameFormat: automatisch abh�ngig vom 'AggType' berechnet
            ...
            else if (isset($Serie['data']))
            {
               foreach($Serie['data'] as $data)
               {
                  if (isset($data['Id']) && isset($data['y']))
                  	die ("Abbruch - Pie['data']: Id und y sind als gleichzeitige Parameter nicht m�glich.");
                  //if (!isset($data['Id']) && !isset($data['y']))
                 	// 	die ("Abbruch - Pie['data']: Id oder y muss definiert sein");
                 	// kann man so nicht pr�fen
               }
            }
            else
				{
					die ("Abbruch - Pie kann nie Daten besitzen. Es muss entweder �ber 'Id' oder �ber 'data' definiert werden.");
				}

			}

			// ge�nderte Werte wieder zur�ckschreiben
			$series[] = $Serie;
		}
		// ge�nderte Werte wieder zur�ckschreiben
	...
	//    pr�fen der AggregatedValues und �bernahme dieser in die Serien
	//    IN: $cfg
	//    OUT: korrigiertes $cfg
	// ------------------------------------------------------------------------
 	function CheckCfg_AggregatedValues($cfg)
 	{
		...
		foreach ($cfg['series'] as $Serie)
		{
			// pr�fen ob f�r die Serie Einstellungen f�r AggregatedValues vorhanden sind,
			// wenn nicht �bernahme aus cfg
			...
			else	// nein -> Daten aus �bergeordneter cfg �bernehmen
				$Serie['AggregatedValues'] = $cfg['AggregatedValues'];

     		// Umrechnen der Tage in Sekunden ... f�r direktes addieren zum Timestamp
			$MinPerTag = 24*60*60;
		...
		// die sind jetzt nicht mehr n�tig.....
...
// ***************************************************************************************************************************
	...
	//
	//    IN: $cfg
	//    OUT: JSON Options String f�r den Bereich 'lang'
	// ------------------------------------------------------------------------
	function GetHtmlForHighchartsSetOptions($cfg)
	{
		...
		IfNotIssetSetValue($cfg['lang']['months'], ['Januar', 'Februar', 'M�rz', 'April', 'Mai', 'Juni', 'Juli', 'August', 'September', 'Oktober', 'November', 'Dezember']);
		IfNotIssetSetValue($cfg['lang']['shortMonths'], ['Jan', 'Feb', 'M�r', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dez']);
	...
	function CreateArrayForSeries($cfg)
	{
		...
		// Daten f�r einzelne Serien erzeugen
		$dataArr = array();
		foreach ($cfg['series'] as $Serie)
		{
			if ($Serie['Ips']['Type'] == 'pie')
			...
			{
            // Daten wurden von extern �bergeben
				if (isset($Serie['data']))
				{
			...
			// nicht f�r JSON ben�tigte Parameter l�schen
	...
	//    IN: $dt
	//			 $serie
	//    OUT: Date-Value f�r Data-String
	// ------------------------------------------------------------------------
	...
	{
		// Werte ersetzten (sinnvoll f�r Boolean, oder Integer - z.B.: T�r/Fenster-Kontakt oder Drehgriffkontakt)
	...
	//    Liest die aktuellen Werte aus den �bergebenen Variablen und erzeugt die Daten f�r das PIE
	//    IN: $cfg, $Serie
	//    OUT: der Data String
	// ------------------------------------------------------------------------
	...
	//    IN: $Serie, $search : "" f�r alle Werte, "Hour", "Day", usw
	...
	function ReadDataFromDBAndCreateDataArray($cfg, $Serie)
	{
		...
		else if ($Serie['AggregatedValues']['MixedMode'])    // im MixedMode werden anfangs alle Werte, dann die Stunden- und zuletzt Tageswerte ausgelesen
		{
			...
			if ($result != false)
			{
				if ($Serie['Ips']['IsCounter']) 						// wenn Z�hler dann immer Agg.Values
		...
		else
		{
         ...
         if  (isset($Serie['AggType']))   // wenn 'AggType' definiert wurde, wird dies vorrangig bearbeitet
         {
			...
			// es wurde noch nichts definiert und es handelt sich um einen Z�hler --> Tageswerte
         if ($Agg == -1 && $Serie['Ips']['IsCounter'])
	         $Agg = 0;

			if ($Agg == -1)
			{
				// Zeitraum ist zu gro� -> nur bis max. Zeitraum einlesen
				if ($Diff > $Serie['AggregatedValues']['NoLoggedValues'])
					$Serie['StartTime'] = $Serie['EndTime'] - $Serie['AggregatedValues']['NoLoggedValues'];
		...
		if ($ReadCurrentValue
			...
			&& !$Serie['Ips']['IsCounter'])				// nicht bei Z�hlervariablen
			{
	...
	//    IN: siehe Parameter
	//    OUT: Vervollst�ndigte Logged Data
	// ------------------------------------------------------------------------
	...
	//    OUT: Highcharts ConfigString f�r Series-Data
	// ------------------------------------------------------------------------
	...
	//    Auslesen von immer wieder ben�tigten Werten aus der Variable
	//    IN: $cfg = Alle Config Daten
	//    OUT: Highcharts ConfigString f�r Tooltip-Formatter (Interaktive Anzeige des Wertes)
	// ------------------------------------------------------------------------
	function CreateTooltipFormatter($cfg)
	{
		...
		foreach ($cfg['series'] as $Serie )
		{
			...
			{
			   // hier wird das VariableCustomProfile aus IPS �bernommen
			   if (!isset($Serie['Unit']))
				{
					...
					if ($IPSProfil != false)
					{
						if (array_key_exists("Associations",$IPSProfil) && count($IPSProfil['Associations'])>0)
						{
							...
							if (!is_array($Serie['ReplaceValues']))         // erzeuge Tooltips vollst�ndig aus VariablenProfil
...
*/
	...
	//    Erzeugt den Tooltip f�r Unter-Elemente
	...
	//    Liest das Variablen Profil der �bergeben Variable aus
	//    Versucht zuerst das eigene und wenn nicht verf�gbar das Standar Profil auszulesen
	...
	function GetIPSVariableProfile($variableId)
	{
		...
		if ($profilName != false)
			return IPS_GetVariableProfile($profilName);  // und jetzt die dazugeh�rigen Daten laden
		else
			return false;
	}
	...
	//
	//    IN: $cfg
	//    OUT: Config Array f�r den Bereich 'chart'
	// ------------------------------------------------------------------------
	...
	//
	//    IN: $cfg
	//    OUT: Config Array f�r den Bereich 'credits'
	// ------------------------------------------------------------------------
	...
	//
	//    IN: $cfg
	//    OUT: Config Array f�r den Bereich 'title'
	// ------------------------------------------------------------------------
	...
	//
	//    IN: $cfg
	//    OUT: Config Array f�r den Bereich 'exporting'
	// ------------------------------------------------------------------------
	...
	//
	//    IN: $cfg
	//    OUT: Config Array f�r den Bereich 'tooltip'
	// ------------------------------------------------------------------------
	...
	//
	//    IN: $cfg
	//    OUT: Config Array f�r den Bereich subtitle
	// ------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------
	// CreateArrayForXAxis
	//    Erzeugen das ArrX-Achsen Strings f�r Highchart-Config
	//    IN: $cfg
	//       es besteht die M�glichkeit den Achsen String bereits im Highchart Format zu hinterlegen
	//       oder die folgenden Parameter als Array einzustellen: Name, Min, Max, TickInterval, Opposite, Unit
	//    OUT: Highcharts String f�r die Achsen
	// ------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------
	// CreateArrayForYAxis
	//    Erzeugen der Y-Achsen Strings f�r Highchart-Config
	//    IN: $cfg
	//       es besteht die M�glichkeit den Achsen String bereits im Highchart Format zu hinterlegen
	//       oder die folgenden Parameter als Array einzustellen: Name, Min, Max, TickInterval, Opposite, Unit
	//    OUT: Highcharts String f�r die Achsen
	...
	// ------------------------------------------------------------------------
	// CreateDateUTC
	//    Erzeugen des DateTime Strings f�r Highchart-Config
	...
	//    OUT: der String �bersetzt ins Deutsche
	// ------------------------------------------------------------------------
	...
	function my_json_encode($cfgArr)
	{
		...
		// workaround damit die " �bergeben werden
		$s = str_replace('\"', "%'%",$s);
...
/*			else if ((substr($item,0,1) == "$" && substr($item,-1) == "$"))
	...
	//    Versuchen Sonderzeichen wie Zeilenumbr�che, Tabs, etc. aus dem �bergebenen String zu entfernen
	//    IN: $str
	//    OUT: $str
	// ------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------
	// IfNotIssetSetValue
	//    pf�ft ob isset($item), wenn nicht wird $value in &$item geschrieben
	//    IN: &$item, $value
	//    OUT: &$item
	// ------------------------------------------------------------------------
	...
	{
		if (!isset($item)
			|| (is_string($item) && $item == ""))   // zus�tzliche Abfrage in 2.01
		{
			$item = $value;
			return false;
		}
